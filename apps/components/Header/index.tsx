...
import { usePathname } from "next/navigation"; // usePathname for Next.js 13+ app router
...
const Header = () => {
  ...
  const openWhatsApp = () => {
    window.open("https://wa.me/081310268786", "_blank");
  };
            ...
            <div className="flex w-full items-center justify-between px-4">
                    ...
                    {menuData.map((menuItem, index) => (
                      <li key={menuItem.id} className="group relative">
                        {menuItem.path ? (
                          <Link
                            href={menuItem.path}
                            className={`flex py-2 text-base text-dark group-hover:opacity-70 dark:text-white lg:mr-0 lg:inline-flex lg:py-6 lg:px-0 ${pathname === menuItem.path ? 'text-[#0095eb] ' : 'text-white'
                              }`} // Apply bg-primary if active
                          >
                            {menuItem.title}
                          </Link>
                        ) : (
              ...
              <div className="flex items-center justify-end pr-16 lg:pr-0">
                <button
                  ...
                  className="rounded-xl bg-primary py-2 px-6 text-base font-semibold text-white duration-300 ease-in-out hover:bg-primary/80"
                >
                  Order Now
                </button>
                <div>
